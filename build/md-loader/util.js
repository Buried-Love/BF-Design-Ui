...
const findContent = (tokens, idx) => {
  ...
  while (index > 0 && (!sourceContent || !descriptionContent)) {
    ...
    if (
      tokens[start] &&
      tokens[start].type === 'fence' &&
      tokens[start].info === 'html'
    ) {
      sourceContent = tokens[start] && tokens[start].content
      tokens[start].demo = true
    } else if (tokens[start] && tokens[start].nesting !== '-1') {
      descriptionContent += tokens[start].content
        ? markdownRender.render(tokens[start].content)
        : ''
...
const renderContent = (tokens, idx) => {
  if (tokens[idx].nesting === 1) {
    ...
    let title = demoInfo && demoInfo.length > 1 ? demoInfo[1] : ''
    let titleHtml = title || ''
    ...
    return `<code-wrap id="${titleId}">
                ...
                <!--bf-design-ui-doc:${sourceContent}:bf-design-ui-doc-->
